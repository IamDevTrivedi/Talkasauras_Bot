...
| `bun build`        | Bundle to `dist/` via `bun build` |
| `bun start`        | Production start from `src/`      |
| `bun lint`         | ESLint auto-fix                   |
| `bun lint:check`   | ESLint check (zero warnings)      |
| `bun format`       | Prettier auto-format              |
| `bun format:check` | Prettier check                    |
| `bun check`        | `lint:check` + `format:check`     |
...
| `bun db:studio`         | Launch Prisma Studio on port 5004 |
